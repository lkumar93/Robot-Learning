 THIS FOLDER CONTAINS THE IMPLEMENTATION OF THE DEEP DETERMINISTIC POLICY GRADIENTS
 ...
 - Install the required libraries using the following commands and set packages using the following commands
	...
	7) sudo pip install tensorflow-gpu
 ...
 - The folder contains 2 source code files, DDPG.py contains the implementation of the DDPG algorithm , Run.py is used to train   
...
-  After this you can run the modules as follows
 ...
 - Usage :
   	...
   	Training -
		...
		python Run.py -t True


	Testing -
		
		python Run.py -t False


	In order to control Roll, Pitch or Thrust, specify that option when initializing the DDPG agent in Run.py
